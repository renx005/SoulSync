...
import { ChevronLeft, Plus, Heart, MessageSquare, Brain, Flame, Book, Globe, Shield, Flag } from "lucide-react";
import { useUser } from "@/contexts/UserContext";
import { PostItem } from "@/pages/community/components/PostItem";
import { useToast } from "@/hooks/use-toast";
import { NewPostSheet } from "@/pages/community/components/NewPostSheet";

export default function ProfessionalCategoryPosts() {
  ...
  // Handle reporting a post (Professional-specific feature)
  const handleReportPost = (postId: string) => {
    toast({
      title: "Post flagged for review",
      description: "Thank you for helping to maintain community standards.",
      variant: "default"
    });
  ...
  // Handle moderating a post (hide post - Professional-specific feature)
              ...
              {posts.map(post => (
                <div key={post.id} className="relative">
                  <PostItem 
                    ...
                    onEdit={user?.id === post.authorId ? handleEditPost : undefined}
                    onDelete={user?.id === post.authorId ? handleDeletePost : undefined}
                    isLiked={likedPosts.includes(post.id)}
                  />
                  
                  {/* Professional moderation buttons */}
                  {user?.role === "professional" && user?.id !== post.authorId && (
